    ...
    <Card>
      ...
      <Modal v-model="modalMap.modalVisible"  :title="modalMap.modalTitle" :loading=true :footer-hide=true>
        <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="100">
            <FormItem label="名称简介" prop="name">
              <template  v-if="formValidate.id">
                <Input v-model="formValidate.name" :maxlength="20" disabled placeholder='请输入名称，不可重复'/>
              </template>
              <template v-else>
                <Input v-model="formValidate.name" :maxlength="20" placeholder='请输入名称，不可重复'/>
              </template>
            </FormItem>
