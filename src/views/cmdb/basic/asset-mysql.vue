  ...
  <Card dis-hover :bordered="false">
    ...
    <div style="margin: 10px; overflow: hidden">
      <div style="float: left">
        <Page :total="i.pageTotal" :current="initPara.page_number" :page-size="initPara.page_size" :page-size-opts="[12, 35, 50, 100, 1000]" 
        show-sizer show-total @on-change="handlerChangePage" @on-page-size-change="handlePageSize" />
      </div>
    </div>
    <!-- MySQL Detail -->
    <div v-if="mysqlData">
      <mysqlDetail :modal-data="modalData" :mysql-data="mysqlData" />
    </div>
    <!-- MySQL Address -->
    ...
    <template v-else-if="addressDialog.show && ['aliyun', 'qcloud', 'volc', 'gcp'].includes(addressDialog.cloudName)">
      <mysqlAddress :dialog="addressDialog" :address-items="addressItems" />
    </template>
...
<script>
import { getMySQLList, optrationMysQL } from '@/api/cloud/mysql'
import mysqlDetail from '../components/mysql-detail.vue'
import mysqlAddress from '../components/mysql-address.vue'
...
export default {
  ...
  methods: {
    async handleGetData() {
      const res = await getMySQLList(this.initPara)
      if (res.code === 0) {
    ...
    handViewAddress(paramsRow) {
      this.addressItems = paramsRow.db_address.items
