  ...
  <Card dis-hover :bordered="false">
    <div>
      ...
      <template v-if="serachType === 'address'">
        <Input v-model="initPara.search_address" style="width: 300px; margin: 2px" clearable placeholder="输入地址模糊查询" @on-change="handleClear" @keydown.enter.native="handleSearch" />
      </template>
      <template v-else>
      ...
      <ButtonGroup>
        ...
        <Button v-if="initPara.search_filter === 'is_showdown'" type="primary" @click="handlefilter('is_showdown')">已关机</Button>
      ...
      <Table ref="selection" stripe size="small" :columns="columns" :data="tableData" :loading="i.tableLoading"
       @on-sort-change="handleSort"  @on-selection-change="handleSelect" />
    </div>
    <!-- 分页 -->
    <div style="margin: 10px; overflow: hidden">
      <div style="float: left">
        <Page :total="i.pageTotal" :current="initPara.page_number" :page-size="initPara.page_size" :page-size-opts="[12, 35, 50, 100]" 
          show-sizer show-total @on-change="handlerChangePage" @on-page-size-change="handlePageSize" />
      </div>
    </div>
    <!-- redisDetail -->
    <div v-if="redisData">
      <redisDetail :modal-data="modalData" :redis-data="redisData" />
    </div>
    <!-- Redis Address -->
    <div v-if="addressDialog.show">
      <redisAddress :dialog="addressDialog" :address-items="addressItems" />
    </div>

    <redis-add :modal-data="modalAdd" @drawer-close="handleClose" @save-data="handleSaveData"/>
  </Card>
</template>

<script>
import { getRedisList, optrationRedis } from '@/api/cloud/redis'
import redisDetail from '../components/redis-detail.vue'
import redisAddress from '../components/redis-address.vue'
import redisAdd from '../components/redis-add.vue'
export default {
  components: {
    redisDetail,
    redisAddress,
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          sortable: true,
          render: (h, params) => {
            if (params.row.cloud_name === 'qcloud') {
              return h('div', [
                h(
                  'Tag',
                  {
            ...
            } else if (params.row.cloud_name === 'aliyun') {
              return h('div', [
                h(
                  'Tag',
                  {
            ...
            } else if (params.row.cloud_name === 'cds') {
              return h('div', [
                h(
                  'Tag',
                  {
            ...
            } else if (params.row.cloud_name === 'volc') {
              return h('div', [
                h(
                  'Tag',
                  {
            ...
            } else if (params.row.cloud_name === 'gcp') {
              return h('div', [
                h(
                  'Tag',
                  {
            ...
            }else if (params.row.cloud_name === 'aws') {
              return h('div', [
                h(
                  'Tag',
                  {
          ...
          render: (h, params) => h('div', [
            h(
              'a',
              {
                ...
                on: {
                  click: () => {
                    this.handViewAddress(params.row)
                  },
                },
              },
              '链接地址',
  ...
  methods: {
    async handleGetData() {
      const res = await getRedisList(this.initPara)
      if (res.code === 0) {
    ...
    handleSaveData (saveData, action, next) {
      optrationRedis(saveData, action).then(res => {
          if (res.code == 0) {
    ...
    handleDetail(paramsRow) {
      this.redisData = paramsRow
      this.modalData.show = true
    },
    handViewAddress(paramsRow) {
      this.addressItems = paramsRow.instance_address.items
      this.addressDialog = {
        show: true,
    ...
    handleDel() {
      this.$Modal.confirm({
        ...
        content: '确定删除已选中的数据? ',
        onOk: () => {
          optrationRedis( { id_list: this.selectList}, 'delete' ).then((res) => {
            if (res.code === 0) {
